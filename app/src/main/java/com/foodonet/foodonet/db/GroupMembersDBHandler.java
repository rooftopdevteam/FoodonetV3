...
public class GroupMembersDBHandler {
    ...
    /**
     * checks in the group db what unique id the user has and returns it
     * @param groupID the group id to check
     * @return the unique user ID of the user in the specific group
     */
    ...
    private boolean isMemberInGroup(long groupID, long memberID){
    ...
    /**
     * @param groupID the group to check
     * @param newPhone the phone number to check
     * @return true if the member is in the group, false if not */
    ...
    /**
     * checks if the user is the group's admin
     * @param groupID the group to check
     * @return true - the user is the group's admin, false if not
     */
    public boolean isUserGroupAdmin(Context context, long groupID) {
        ...
        String[] whereArgs = {String.valueOf(groupID),
                String.valueOf(CommonMethods.getMyUserID(context)),
                String.valueOf(CommonConstants.VALUE_TRUE)};
        Cursor c = context.getContentResolver().query(FoodonetDBProvider.MembersDB.CONTENT_URI,selection,where,whereArgs,null);
        boolean found = false;
        if(c!=null && c.moveToNext()){
            found = true;
    ...
    /**
     * deletes a specific member of the group using the unique user ID
     * @param uniqueID the unique user ID to delete (not the regular user ID, but a unique on for a group member in a group)
     */
