...
class FoodonetDBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        sql = String.format("CREATE TABLE %1$s(%2$s INTEGER PRIMARY KEY AUTOINCREMENT,%3$s INTEGER,%4$s INTEGER,%5$s INTEGER, %6$s TEXT, %7$s TEXT, %8$s TEXT)",
                ...
                FoodonetDBProvider.RegisteredUsersDB.REGISTERED_USER_ACTIVE_DEVICE_UUID_COLUMN,FoodonetDBProvider.RegisteredUsersDB.REGISTERED_USER_NAME_COLUMN,
                FoodonetDBProvider.RegisteredUsersDB.REGISTERED_USER_PHONE_COLUMN);
        sqLiteDatabase.execSQL(sql);
        sql = String.format("CREATE TABLE %1$s(%2$s INTEGER PRIMARY KEY AUTOINCREMENT,%3$s INTEGER,%4$s INTEGER,%5$s TEXT,%6$s INTEGER, %7$s TEXT)",
                FoodonetDBProvider.NotificationsDB.TABLE_NAME,FoodonetDBProvider.NotificationsDB._ID_COLUMN,FoodonetDBProvider.NotificationsDB.ITEM_ID_COLUMN,
                FoodonetDBProvider.NotificationsDB.NOTIFICATION_TYPE_COLUMN,FoodonetDBProvider.NotificationsDB.NOTIFICATION_NAME_COLUMN,
                FoodonetDBProvider.NotificationsDB.NOTIFICATION_RECEIVED_TIME_COLUMN,FoodonetDBProvider.NotificationsDB.NOTIFICATION_IMAGE_FILE_NAME_COLUMN);
        sqLiteDatabase.execSQL(sql);
    ...
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int i, int i1) {
        if(i==1){
            String sql = String.format("ALTER TABLE %1$s ADD COLUMN %2$s TEXT",
                    FoodonetDBProvider.NotificationsDB.TABLE_NAME,FoodonetDBProvider.NotificationsDB.NOTIFICATION_IMAGE_FILE_NAME_COLUMN);
            sqLiteDatabase.execSQL(sql);
        }
